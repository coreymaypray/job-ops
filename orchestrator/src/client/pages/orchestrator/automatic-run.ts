...
export function deriveExtractorLimits(args: {
  ...
  const includesDice = args.sources.includes("dice");
...
export function calculateAutomaticEstimate(args: {
  ...
  const hasDice = sources.includes("dice");
  ...
  const hiringCafeCap = hasHiringCafe
    ...
    : 0;
  const diceCap = hasDice ? limits.jobspyResultsWanted * termCount : 0;

  const discoveredCap =
    jobspyCap + gradcrackerCap + ukvisaCap + adzunaCap + hiringCafeCap + diceCap;
